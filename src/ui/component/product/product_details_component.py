...
class ProductDetailsComponent(BaseComponent):
    ...
    @step_log("Add {count} [{self._component_title}] product(s) to cart")
    ...
    def check_product_has_data(self, product_info: ProductDTO):
    ...
    def check_product_has_category(self, category: CategoryDTO) -> None:
        group = category.usertype.user_type.value
    ...
    def check_product_has_price(self, price: PriceDTO) -> None:
        with step_log.log(f"Check product [{self._component_title}] has price: {price.get_amount_text()}"):
            self._locator.category().should_have_text(price.get_amount_text())
    ...
    def check_product_has_condition(self, condition: str):
        self._locator.availability().should_have_text(f"Condition: {condition}")

    @step_log.log("Check product [{self._component_title}] has condition: {brand}")
    def check_product_has_brand(self, brand: str):
        self._locator.availability().should_have_text(f"Condition: {brand}")
